...
aus:%
{\lstinputlisting[firstline=10]{rk_tableaus.py}}%
...
die Raumkoordinaten äquidistant mit Abstand $h$, \dh, wir setzen
...
werden, da ja $\widehat{\nabla \Phi} = i n \omega \hat \Phi$ (siehe
\eqref{eq:dfft}). Zweimalige Anwendung ergibt
\begin{equation}
  \label{eq:poissonft}
...
der diskreten FFT in den Fourierraum, lösen dort trivial die
Poissongleichung, und transformieren $\hat\Phi$ zurück. Dabei ist zu
beachten, dass das \emph{diskretisierte} $\omega = 2\pi h / L = 2\pi / N$
(siehe \eqref{eq:dft}).
...
\lstinputlisting[style=multipage,firstline=10, caption={Python-Code zur
