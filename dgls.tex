...
Wir suchen suchen eine diskretisierte Näherung $y_n \approx y(t_n)$
...
Im Folgenden wird angerissen, wie die Finite-Elemente-Methode
...
die Raumkoordinaten äquidistant mit Abstand $h$. Für die zweite
...
\begin{equation}
  \frac{\delta}{\delta t} p(x_k, t) =
  \frac{D}{h^2} \left[p(x_{k-1}, t) - 2 p(x_k, t) + p(x_{k+1}, t)\right].
\end{equation}
Aus der partiellen Differentialgleichung ist eine gewöhnliche
Differentialgleichung in $t$ geworden, mit $N$ Variablen
$p(x_1,t),\ldots,p(x_N, t)$, wobei $N$ die Anzahl der Ortspunkte
ist. Diese DGL lösen wir nun mit dem Runge-Kutta-Verfahren. Das hat
den großen Vorteil, dass wir die Verteilungen $p(x_k, t)$, die bei
...
betrachten dabei nur das Intervall $[-10,10)$, dass wir mit $N=200$
...
Funktion außerhalb des Intervalls auf 0. Für hinreichend großes $L$
nähert die Lösung damit das klassische, freie Problem.
...
\begin{equation}
  \int_{-L}^L p(x, t)\, dx \approx \sum h p(x_n, t)
\end{equation}
