...
for c, res in (((0.5, 0.5, 0), (0.0, 0.0, 1.0)),
    ...
    np.testing.assert_allclose(
        answer,
        np.array(res),
        err_msg="Simplex hat das Minimum nicht gefunden")
...
with np.testing.assert_raises_regex(Exception, "zulaessige Menge unbeschraenkt!"):
    answer = simplex(np.array(c), A, b)
...
with np.testing.assert_raises_regex(Exception, "zulaessige Menge ist leer!"):
    answer = simplex(np.array(c), A, b)
