...
bezeichnet.  Die bekannten schnellen FFT-Routinen lassen sich also
...
$f_\text{Nyquist}=\frac{1}{2\Delta}$ abgetastet werden können, ähnlich
wie im periodischen Fall.
...
bzw. $\mean{(A-\mean{A})(B-\mean{B})}=0$ erfüllen. Sind nun $A=A(t)$
und $B=B(t)$ zeitliche Messreihen eines ergodischen stochastischen
Prozesses mit verschwindenden Mittelwerten, $\mean{A} = \mean{B} = 0$,
so ergeben sich die Mittelwerte als Zeitmittelungen, also Integrale
über die Zeit:
\begin{equation}
  \mean{A\cdot B} \stackrel{!}{=} C(A,B) := \int_{-\infty}^{\infty} A(t)B(t) dt
\end{equation}
Ergodisch bedeutet hier einfach, dass der Prozess bei einer langen
zeitlichen Messung alle möglichen Ergebnisse einmal besuchen wird,
sprich, dass eben der zeitliche Mittelwert gleich dem
Ensemble-Mittelwert ist.

Im allgemeinen wird man allerdings nicht erwarten, dass sich die
Änderung einer Observablen unmittelbar in einer anderen niederschlägt,
sondern erst nach einer Zeit $\tau$.  Um nun die Korrelation zwischen
$A$ und $B$ mit einem zeitlichen Versatz von $\tau$ zu betrachten,
definiert man daher die \emph{\keyword{Kreuzkorrelationsfunktion}} als
\begin{equation}
  C(A,B)(\tau) = \int_{-\infty}^{\infty} A(t)B(t+\tau) dt =
  \bigl(A \star B(-\cdot)\bigr)(-\tau) = \bigl(A(-\cdot) \star B\bigr)(\tau).
\end{equation}
Als Faltung kann die
Kreuzkorrelationsfunktion effizient im Frequenzraum bestimmt werden:
\begin{equation}
  C(A,B) = \sqrt{2\pi}\FT^{-1}
  \bigl(\FT(A)(-\omega)\FT(B)(\omega)\bigr).
\end{equation}
...
Die Bedingung $\mean{A} = \mean{B} = 0$ kann stets durch Abziehen des
Mittelwerts vom Signal erreicht werden. Ist dieser nicht analytisch
bekannt, kann er numerisch durch Mittelung der Daten bestimmt
werden. Oft wird aber selbst bei Observablen, die aus Symmetriegründen
einen Mittelwert von Null haben müssen, der numerische gemessene
Mittelwert abgezogen. Diese übliche Praxis ist aber falsch, und führt
zu einer systematischen Unterschätzungen der Korrelationen im System!
...
es dauert, bis das Signal nicht mehr von seinem vorherigen Wert
...
In diesem letzten Abschnitt zur Signalanalyse geht es darum, wie der
Messfehler bei einem stochastischen Signal abgeschätzt werden
kann. Wir betrachten also eine Messreihe $x_i$, $i=1(1)n$, die
verschiedene Realisationen einer Zufallsvariablen $x$ darstellen. Der
Mittelwert dieser Variablen lässt sich dann bekanntermaßen als
\begin{equation}
  \label{eq:mean}
