...
Bei der Optimierung betrachtet man eine Funktion $f:M\to \RR$, die
\emph{Zielfunktion}, mit einer beliebigen \emph{zulässigen Menge} $M$.
Gesucht ist $x\in M$, so dass $f(x) \le f(x')$ für alle $x'\in M$. Man
schreibt kurz
\begin{equation}
  \label{eq:min}
  x = \argmin_{x'\in M} f(x') \quad\text{und}\quad f(x) = \min_{x'\in M} f(x')
\end{equation}
...
\begin{equation}
  \label{eq:fit}
  \argmin_{v} \sum_{i=1}^n (f_v(x_i) - y_i)^2.
\end{equation}
...
\begin{equation}
  \label{eq:min2}
  \argmin_{(v_x,v_c)\in\RR^{m+1}} \sum_{i=1}^n (v_x^T x_i -
  v_c - y_i)^2 = \argmin_{v\in\RR^{m+1}} \norm{A v - b}_2.
\end{equation}
mit
\begin{equation*}
  b =
...
\begin{equation}
  \label{eq:mininf}
  \argmin_{v\in\RR^{m+1}} \norm{A v -
    b}_\infty = \argmin_{v\in\RR^{m+1}} \max_{i=1}^n \abs{(A v)_i - y_i}.
\end{equation}
...
\begin{equation}
  ...
  \argmin_v (0,\ldots,\,0,\,1)^T v\quad\text{unter der Bedingung}\;
  \begin{pmatrix}
    A  & e\\
    -A & e
  \end{pmatrix} v \ge
...
Dann ist
\begin{equation}
  (A^TA)^{-1}A^Tb = (R^TQ^TQ R)^{-1}A^Tb = (R^TR)^{-1}A^Tb =
...
m$-Dreiecksmatrix $R_1$ und eine orthonormale Matrix $Q$. Dann gilt entsprechend
\begin{equation}
  A^T(AA^T)^{-1}b = A^T(R^TQ^TQ R)^{-1}b =
...
\begin{equation}
  Ax = b \quad\iff\quad x = \argmin f(x') = \frac{1}{2}x^TAx' - x'^Tb.
\end{equation}
...
\begin{equation}
  \argmin_{x\in M} f(x) = \argmin_{x\in\RR^n} \left[f(x) + \sum_{i=1}^m \min(0,
  g_i(x))^2\right].
\end{equation}
...
{\samepage\vspace{0.2em}% für die TikZ-Deko
  \begin{equation}
    ...
    \argmin_x (\tikzlabel{vpl}0,\ldots,\,0,\,1\tikzlabel{vpr},
...
{\samepage\vspace{0.2em}\begin{equation}
  \argmin_z (\tikzlabel{xl}0,\ldots,\,0\tikzlabel{xr},
