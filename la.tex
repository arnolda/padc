...
die Matrix zum Beispiel entlang der ersten Zeile entwicklen, und $n$
...
brauchen wir noch ein anderes Verfahren, das alle Eigenwerte
liefert. Dieser Abschnitt führt hierzu den QR-Algorithmus von Francis
und Kublanovskaya ein.
...
einer Matrix mit $2\times 2$-Blöcke auf der Diagonalen, die jeweils
...
$Q^{(k)}$. Einfacher ist es aber, eine sogenannte
\emph{\keyword{inverse Iteration}} durchzuführen. Ist $\lambda$ ein
Eigenwert der Matrix $A\in\CC^{n,n}$, so ist $A-\lambda I$
singulär. Trotzdem löst man
\begin{align}
  (A - \lambda I) \tilde{x}^{(i+1)} = x^{(i)}
\end{align}
durch Gaußelimination mit Pivotisierung und setzt
\begin{equation}
  x^{(i+1)} = \frac{\tilde{x}^{(i+1)}}{\norm{\tilde{x}^{(i+1)}}_2}.
\end{equation}
Trifft man bei der Gaußelimination auf eine exakte numerische Null,
wird diese durch eine sehr kleine Zahl ersetzt. Da letztlich nur die
...
def eigenwerte(A, tolerance):
    ...
    while not converged:
        ...
        if max([abs(Ak[i,k]) for i in range(A.shape[0]) \
                             for k in range(i)]) < tolerance:
            converged = True
    return Ak
\end{lstlisting}
Die Routine liefert die gesamte Beinahe-Dreiecksmatrix zurück, die auf
der Diagonalen die Eigenwerte zeigt. \argd{tolerance} gibt dabei den
...
unter der Diagonalen nicht verschwinden, das Kriterium muss dann
entsprechend angepasst werden, ansonsten funktioniert der Algorithmus
genauso.
...
Um die zugehörigen Eigenvektoren zu bestimmen, benutzen wir die
inverse Iteration, die in diesem Fall allerdings für beide
Eigenvektoren innerhalb von nur einem Schritt auf Maschinengenauigkeit
konvergiert.

Ist die Matrix $A\in\RR^{10,10}$ symmetrisch mit Standardzufallszahlen
gefüllt, konvergiert das Verfahren in etwa 200 Schritten so, dass
unterhalb der Diagonalen keine Werte betragsmäßig größer als $10^{-5}$
stehen. Aber auch hier konvergiert die inverse Iteration innerhalb
eines Schritts praktisch bis auf Maschinengenauigkeit gegen den
